function RankForm({
  ...
  return (
    <form onSubmit={onSubmit} className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
      <h2 className="text-xl font-bold text-gray-900">{exam.title || `${exam.name} Rank Predictor`}</h2>
      ...
      <button
        type="submit"
        disabled={loading}
        className="mt-6 rounded-lg bg-primary-navy px-5 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-primary-navy/90 disabled:opacity-50"
      >
