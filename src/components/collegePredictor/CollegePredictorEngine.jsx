...
import {
  COLLEGE_PREDICTOR_LEAD_OCCUPATION,
  COLLEGE_PREDICTOR_LEAD_UTM,
} from '../../utils/collegePredictorLeadConstants';
import {
  swBtnGhost,
  swBtnPrimary,
  swErrorBox,
...
import PredictorOtpDigitInputs from '../rankPredictor/PredictorOtpDigitInputs';
import {
  getEntranceExamMeta,
  ENTRANCE_EXAMS,
  rankToCutoff,
...
import {
  ...
  KeamPredictorForm,
  WbjeePredictorForm,
} from '../Counsellor/CollegePredictor';
...
const DEFAULT_EXAMS_PATH = '/counsellor/tools/college-predictor';
...
/**
 * Shared college predictor engine.
 * @param {{ predictApi?: Function, examsListPath?: string, requireLeadGate?: boolean, hideChrome?: boolean, onMatchCount?: (n:number)=>void }} [props]
 */
export default function CollegePredictorEngine({
  predictApi: predictApiProp,
  examsListPath = DEFAULT_EXAMS_PATH,
  ...
  const predictApi = predictApiProp || getPredictedColleges;
  const COLLEGE_PREDICTOR_EXAMS_PATH = examsListPath;
  ...
  const [wizardStep, setWizardStep] = useState(/** @type {'form' | 'lead'} */ ('form'));
  const [otpUnlocked, setOtpUnlocked] = useState(!requireLeadGate);
  ...
  const pendingActionRef = useRef(/** @type {null | (() => void | Promise<void>)} */ (null));
  const verifiedPhoneRef = useRef('');
  const leadFilterSnapshotRef = useRef('');
  ...
  const fetchMhtColleges = useCallback(
    async (pageOffset = 0, append = false) => {
      ...
      if (!append) {
        ...
        if (requireLeadGate && verifiedPhoneRef.current) {
          const matchTotal = data.total_no_of_colleges ?? 0;
          if (typeof onMatchCount === 'function') onMatchCount(matchTotal);
          saveCollegePredictorPrediction(verifiedPhoneRef.current, {
            exam,
            matchCount: matchTotal,
          }).then((syncRes) => {
            if (!syncRes?.success) {
              setPredictionSyncError(syncRes?.message || 'Could not save prediction for admin.');
            }
          });
        }
      }
    ...
    [examMeta, mhtForm, validateMht, requireLeadGate, exam, onMatchCount, predictApi]
  );
  ...
  const fetchTneaColleges = useCallback(
    ...
    [examMeta, tneaForm, validateTnea, requireLeadGate, exam, onMatchCount, predictApi]
  );
  ...
  const fetchKeamColleges = useCallback(
    ...
    [examMeta, keamForm, validateKeam, requireLeadGate, exam, onMatchCount, predictApi]
  );
  ...
  const fetchWbjeeColleges = useCallback(
    ...
    [examMeta, wbjeeForm, validateWbjee, requireLeadGate, exam, onMatchCount, predictApi]
  );


  const requestSearch = useCallback(
    (action, filterSnapshot = '') => {
      if (!requireLeadGate || otpUnlocked) {
        return action();
      }
      leadFilterSnapshotRef.current = filterSnapshot || '';
  ...
  const handleSendOtp = useCallback(async () => {
    ...
    setLoadingOtp(true);
    try {
      const otpRes = await sendOtp(name, normalizedPhone, COLLEGE_PREDICTOR_LEAD_OCCUPATION);
      if (!otpRes.success) {
        setLeadError(otpRes.message || 'Could not send OTP. Please try again.');
        return;
      }
      const s1 = await saveStep1(name, normalizedPhone, COLLEGE_PREDICTOR_LEAD_OCCUPATION, COLLEGE_PREDICTOR_LEAD_UTM, {
        collegePredictorLead: {
          exam,
          ...(leadFilterSnapshotRef.current ? { filterSnapshot: leadFilterSnapshotRef.current } : {}),
        },
      });
      if (!s1.success) {
        console.warn('[CollegePredictorEngine] saveStep1:', s1.message);
      }
      setOtpSent(true);
      setSuccessMessage('OTP sent. Enter the 6-digit code to see your college matches.');
      setOtp(['', '', '', '', '', '']);
    } catch (err) {
      setLeadError(err.message || 'Could not send OTP.');
    } finally {
      setLoadingOtp(false);
    }
  }, [fullName, phone, exam]);

  const handleVerifyAndPredict = useCallback(async () => {
    setOtpError('');
    setLeadError('');
    const normalizedPhone = normalizePhoneDigits(phone);
    ...
    setVerifying(true);
    try {
      const v = await verifyOtp(normalizedPhone, otpCode);
      if (!v.success || v.data?.verified !== true) {
        setOtpError(v.message || 'Invalid OTP. Please try again.');
        return;
      }
      const s2 = await saveStep2(normalizedPhone, COLLEGE_PREDICTOR_LEAD_UTM);
      if (!s2.success) {
        console.warn('[CollegePredictorEngine] saveStep2:', s2.message);
      }
  ...
  const handleMhtSubmit = useCallback(() => {
    ...
    setColleges([]);
    setOffset(0);
    requestSearch(() => fetchMhtColleges(0, false), getMhtFilterSnapshot(mhtForm));
  }, [validateMht, fetchMhtColleges, requestSearch, mhtForm]);
  ...
  const handleTneaSubmit = useCallback(() => {
    ...
    setColleges([]);
    setOffset(0);
    requestSearch(() => fetchTneaColleges(0, false), getTneaFilterSnapshot(tneaForm));
  }, [validateTnea, fetchTneaColleges, requestSearch, tneaForm]);
  ...
  const handleKeamSubmit = useCallback(() => {
    ...
    setColleges([]);
    setOffset(0);
    requestSearch(() => fetchKeamColleges(0, false), getKeamFilterSnapshot(keamForm));
  }, [validateKeam, fetchKeamColleges, requestSearch, keamForm]);
  ...
  const handleWbjeeSubmit = useCallback(() => {
    ...
    setColleges([]);
    setOffset(0);
    requestSearch(() => fetchWbjeeColleges(0, false), getWbjeeFilterSnapshot(wbjeeForm));
  }, [validateWbjee, fetchWbjeeColleges, requestSearch, wbjeeForm]);
  ...
  const fetchColleges = useCallback(
    ...
    [exam, examMeta, filters, validate, predictApi, requireLeadGate, onMatchCount]
  );
  ...
  const handleJeeSubmit = useCallback(async () => {
    ...
    requestSearch(run, getJeeFilterSnapshot(jeeForm));
  }, [jeeForm, validateJee, runJeeFetch, requestSearch, requireLeadGate, exam]);
  ...
  const handleSubmit = useCallback(() => {
    ...
    setColleges([]);
    setOffset(0);
    requestSearch(() => fetchColleges(0, false), getPredictorFilterSnapshot(filters));
  }, [fetchColleges, requestSearch, validate, filters]);
  ...
  useEffect(() => {
    if (!isJee) return;
    if (requireLeadGate && !otpUnlocked) return;
  ...
  const showForm = !requireLeadGate || wizardStep === 'form';
  const showLead = requireLeadGate && wizardStep === 'lead';
      ...
      {showLead && (
        <div className="space-y-4 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm sm:p-6">
          ...
          <p className="text-sm font-medium text-slate-600">
            Enter your details to receive an OTP. After verification, your college matches will load automatically.
          </p>
          <label className={`block ${swLabel}`}>
            <span>Full name</span>
            <input
              type="text"
              value={fullName}
          ...
          {leadError && <p className={swErrorBox}>{leadError}</p>}
          {successMessage && <p className={swSuccess}>{successMessage}</p>}
          <button
            type="button"
          ...
          {otpSent && (
            <div className="space-y-3 pt-2">
              <p className="text-sm font-medium text-slate-600">Enter OTP</p>
              ...
              />
              {otpError && <p className={swErrorBox}>{otpError}</p>}
              <button
                type="button"
                onClick={handleVerifyAndPredict}
                disabled={verifying}
                className={swBtnPrimary}
              >
                {verifying ? 'Verifying…' : 'Verify & show matches'}
              </button>
            </div>
          )}
        </div>
