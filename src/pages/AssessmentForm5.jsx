...
import { sendOtp, verifyOtp, submitAssessment5, getAssessmentQuestions } from '../utils/api';
...
export default function AssessmentForm5() {
  ...
  useEffect(() => {
    let cancelled = false;
    getAssessmentQuestions(5).then((res) => {
      if (cancelled) return;
      ...
      if (res.success && res.data?.questions?.length) {
        ...
        setSections([{ title: 'Session - 5', questions }]);
