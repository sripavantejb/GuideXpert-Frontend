...
import { sendOtp, verifyOtp, submitAssessment4 } from '../utils/api';
...
function getInitialAnswers() {
  const initial = {};
  ASSESSMENT_SECTIONS_4.forEach((section) => {
...
export default function AssessmentForm4() {
  ...
  const flatQuestions = useMemo(
    () =>
      ASSESSMENT_SECTIONS_4.flatMap((s) =>
  ...
  const questionTextMap = useMemo(() => {
    const map = {};
    ASSESSMENT_SECTIONS_4.forEach((s) => {
  ...
  const handleSendOtp = async (e) => {
    ...
    setLoading(true);
    try {
      const result = await sendOtp(name.trim(), normalizedPhone, 'Counsellor Assessment 4');
      if (result.success) {
  ...
  const handleSubmitAssessment = async (e) => {
    ...
    setSubmitting(true);
    try {
      const result = await submitAssessment4(name.trim(), normalizedPhone, answers);
      if (result.success) {
        setSubmittedResult({
          score: result.data?.score ?? 0,
          maxScore: result.data?.maxScore ?? MAX_SCORE_4,
    ...
    <div className="assessment-page-wrap py-8 px-4">
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-6">
          ...
          <p className="text-gray-600 mt-1">Counsellor Assessment 4</p>
        </div>
          ...
          {step === 1 && (
            <>
              ...
              <form onSubmit={handleSendOtp} className="space-y-4">
                <div>
                  <label htmlFor="assessment4-name" className="block text-sm font-medium text-gray-700 mb-1">
                ...
                <div>
                  <label htmlFor="assessment4-mobile" className="block text-sm font-medium text-gray-700 mb-1">
                    Mobile Number <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="tel"
                    id="assessment4-mobile"
                    value={mobileNumber}
              ...
              <div className="p-6 sm:p-8">
                <div className="text-center">
                  ...
                  <p className="mt-3 text-sm text-gray-500">
                    Thank you for completing the counsellor assessment 4.
                  </p>
                </div>
      ...
      <SuccessPopup
        ...
        maxScore={submittedResult?.maxScore ?? MAX_SCORE_4}
        message="Thank you for completing the Counsellor Assessment 4."
      />
    </div>
  );
}
