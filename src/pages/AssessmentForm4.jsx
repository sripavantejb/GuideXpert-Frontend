...
import { sendOtp, verifyOtp, submitAssessment4, getAssessmentQuestions } from '../utils/api';
...
export default function AssessmentForm4() {
  ...
  useEffect(() => {
    let cancelled = false;
    getAssessmentQuestions(4).then((res) => {
      if (cancelled) return;
      ...
      if (res.success && res.data?.questions?.length) {
        ...
        setSections([{ title: 'Session - 4', questions }]);
