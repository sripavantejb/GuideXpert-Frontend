...
const COPY_FIELDS = [
  ...
  { key: 'sessionRating', label: 'Rating' },
  { key: 'suggestions', label: 'Suggestions' },
  { key: 'createdAt', label: 'Submitted' },
];

function getResponseCellValue(row, key) {
  const v = row[key];
...
export default function TrainingFormResponses() {
  const { logout } = useAuth();
  const [records, setRecords] = useState([]);
  const [pagination, setPagination] = useState({ page: 1, limit: 25, total: 0, totalPages: 1 });
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');
  const [stats, setStats] = useState({ total: 0, bySessionRating: {} });
  ...
  useEffect(() => {
    ...
    getTrainingFormResponses(params, getStoredToken()).then((result) => {
      if (cancelledRef.current) return;
      ...
      if (!result.success) {
        ...
        setError(result.message || 'Failed to load training form responses');
        return;
      }
      ...
      setStats({
        ...
        bySessionRating: responseStats.bySessionRating || {}
      });
    });
    return () => { cancelledRef.current = true; };
  }, [viewAll, pagination.page, pagination.limit, filters.q, filters.from, filters.to, filters.sessionRating, logout]);
  ...
  const clearFilters = () => {
    setFilters({ q: '', from: '', to: '', sessionRating: '' });
    setPagination((prev) => ({ ...prev, page: 1 }));
  };
  ...
  const prepareCopyRecords = async () => {
    ...
    const result = await fetchAllPaginatedRows((page, limit) =>
      getTrainingFormResponses({ ...baseParams, page, limit }, getStoredToken())
    );
    ...
    if (!result.success) {
      ...
      setError(r?.message || 'Failed to load responses for copy');
      return;
    }
    ...
    <div className="max-w-[1400px] mx-auto px-1">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Training Form Responses</h1>
            <p className="text-sm text-gray-500 mt-0.5">View and manage training form (interest) submissions</p>
          </div>
        </div>
      </div>
      ...
      {!loading && (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-6 gap-4 mb-6">
          ...
          {[1, 2, 3, 4, 5].map((rating) => (
            <div key={rating} className="rounded-2xl bg-white border border-gray-200 shadow-sm overflow-hidden">
              ...
              <div className="p-4 flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs font-medium text-gray-500">Rating {rating}</p>
                  <p className="text-xl font-bold text-gray-900">{stats.bySessionRating?.[rating] ?? 0}</p>
                </div>
              </div>
            </div>
          ))}
      ...
      <div className="rounded-2xl border border-gray-200 bg-white shadow-lg overflow-hidden mb-6">
        ...
        <div className="p-5">
          <div className="flex flex-wrap items-center gap-3">
            ...
            />
            <select
              value={filters.sessionRating}
              onChange={(e) => handleFilterChange('sessionRating', e.target.value)}
            ...
            >
              ...
              {[1, 2, 3, 4, 5].map((n) => (
                <option key={n} value={n}>Rating {n}</option>
              ))}
            </select>
            <label className="inline-flex items-center gap-2 cursor-pointer text-sm text-gray-700 ml-2 pl-2 border-l border-gray-200">
              <input
                ...
                aria-label="View all responses in one list"
              />
              View all
            </label>
            <button
              ...
              <table className="min-w-[900px] w-full text-left text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200">
                    ...
                    <th className="px-4 py-3.5 font-semibold text-gray-700 text-xs uppercase tracking-wider text-center">Rating</th>
                    <th className="px-4 py-3.5 font-semibold text-gray-700 text-xs uppercase tracking-wider">Suggestions</th>
                  ...
                  {records.map((row) => (
                    <Fragment key={row.id}>
                      <tr className="hover:bg-[#003366]/4 transition-colors">
                        <td className="px-4 py-3 align-middle font-medium text-gray-900">{row.fullName || '—'}</td>
                        ...
                        <td className="px-4 py-3 align-middle text-center">
                          <span className="inline-flex items-center justify-center min-w-8 px-2 py-0.5 rounded-md bg-amber-100 text-amber-800 text-xs font-medium">
                            {row.sessionRating ?? '—'}
                          </span>
                        </td>
                        <td className="px-4 py-3 align-middle text-gray-600 max-w-[200px] truncate text-xs" title={row.suggestions || ''}>{row.suggestions || '—'}</td>
                          ...
                          <td colSpan={8} className="p-0">
                            <div className="bg-gray-50/80 border-t border-gray-100 px-4 py-4">
                              <div className="rounded-xl bg-white border border-gray-200 p-4 shadow-sm">
                                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Suggestions / Additional comments</p>
                                <p className="text-gray-800 text-sm whitespace-pre-wrap">{row.suggestions || '—'}</p>
                              </div>
                            </div>
                          </td>
                        </tr>
              ...
              {viewAll ? (
                <p className="text-sm text-gray-500">
                  {pagination.total > ADMIN_VIEW_ALL_LIMIT
                    ? `Showing first ${ADMIN_VIEW_ALL_LIMIT.toLocaleString()} of ${pagination.total} responses`
                    : `Showing all ${pagination.total} responses`}
                </p>
              ) : (
                <>
