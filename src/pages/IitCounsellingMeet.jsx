...
import { registerForIitMeet, sendOtp, verifyOtp } from '../utils/api';

const IIT_MEET_LINK = 'https://meet.google.com/woo-pufk-bgz';
...
/** Public attendance page for the IIT counselling meet.
 *
 *  Mirrors /meet's name + OTP UX, but intentionally has NO eligibility gate:
 *    - no FormSubmission lookup
 *    - no demo-window / time-window check
 *    - no booking / training-record check
 *
 *  After OTP verify, we record the attendee in `iitmeetattendances` via
 *  POST /api/iit-meet/register, then hard-redirect to IIT_MEET_LINK. */
export default function IitCounsellingMeet() {
  ...
  const handleSendOtp = async (e) => {
    ...
    try {
      const result = await sendOtp(name.trim(), cleanPhone, 'IIT Meet Attendee');
  ...
  const attemptJoinMeet = async (phone, displayName) => {
    if (joiningRef.current) return;
    joiningRef.current = true;
    try {
      const reg = await registerForIitMeet(displayName, phone);
      if (!reg.success) {
        joiningRef.current = false;
        setSubmitError(reg.message || 'Could not complete registration. Please try again.');
        return;
      }
      window.location.href = IIT_MEET_LINK;
    } catch {
      joiningRef.current = false;
  ...
  const handleResendOtp = async () => {
    ...
    try {
      const result = await sendOtp(name.trim(), phone, 'IIT Meet Attendee');
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-lg max-w-md w-full p-6 sm:p-8">
        <div className="text-center mb-6">
          ...
          <p className="text-gray-600 mt-1">IIT Counselling Meet</p>
        </div>
        ...
        {step === 1 && (
          <>
            <h2 className="text-lg font-semibold text-gray-900 mb-1">Join IIT Counselling Meet</h2>
        ...
        {step === 1 && (
          <form onSubmit={handleSendOtp} className="space-y-4">
            <div>
              <label htmlFor="iit-meet-name" className="block text-sm font-medium text-gray-700 mb-1">
            ...
            <div>
              <label htmlFor="iit-meet-mobile" className="block text-sm font-medium text-gray-700 mb-1">
