...
import { getAssessmentQuestions } from '../../../utils/api';

/**
 * Reusable webinar assessment for types 1–5. Fetches questions from GET /api/assessments?type=X.
 */
export default function WebinarAssessment({ assessmentType, onComplete, nextLabel, onGoNext }) {
  const type = typeof assessmentType === 'number' ? assessmentType : parseInt(String(assessmentType), 10);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    getAssessmentQuestions(type).then((res) => {
      if (cancelled) return;
      setLoading(false);
  ...
  const handleSubmit = useCallback(() => {
    if (!allAnswered || !questions.length) return;
    let score = 0;
  ...
  if (loading) {
    return (
      <div className="flex flex-col h-full min-h-[360px] items-center justify-center p-6 text-gray-500">
        Loading questions...
      </div>
    );
  }
  if (error || questions.length === 0) {
    return (
      <div className="flex flex-col h-full min-h-[360px] items-center justify-center p-6 text-red-600">
    ...
    return (
      <div className="flex flex-col h-full min-h-0">
        <div className="flex flex-col items-center px-6 py-6 text-center">
          ...
          <h2 className="text-xl font-semibold text-primary-navy">Assessment {type} completed</h2>
