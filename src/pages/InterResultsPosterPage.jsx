...
import InterResultsPosterPreview, {
  INTER_RESULTS_POSTER_WIDTH as POSTER_WIDTH,
  INTER_RESULTS_POSTER_HEIGHT as POSTER_HEIGHT,
  INTER_RESULTS_POSTER_EXPORT_LAYOUT,
} from '../components/Counsellor/InterResultsPosterPreview';
...
function loadInterResultsPosterImage() {
  return new Promise((resolve, reject) => {
    ...
    img.src = '/inter-results-poster.svg';
  });
}
...
function drawInterResultsPosterToCanvas(img, fullName, mobileNumber, scale = 2) {
  ...
  const sx = scale;
  const nl = INTER_RESULTS_POSTER_EXPORT_LAYOUT.name;
  const pl = INTER_RESULTS_POSTER_EXPORT_LAYOUT.phone;
...
export default function InterResultsPosterPage() {
  ...
  const handlePng = async () => {
    ...
    setGenerating(true);
    try {
      const img = await loadInterResultsPosterImage();
      const c = drawInterResultsPosterToCanvas(img, displayName, mobile10, 2);
      const url = c.toDataURL('image/png');
      if (isIOSDevice()) {
        setIosResult({ url, type: 'image' });
      } else {
        const fn = `GuideXpert-Inter-Results-Poster-${safeFilename(displayName)}-${Date.now()}.png`;
  ...
  const handlePdf = async () => {
    ...
    try {
      ...
      if (isIOSDevice()) {
        ...
        const fn = `GuideXpert-Inter-Results-Poster-${safeFilename(displayName)}-${Date.now()}.pdf`;
        ...
        {iosResult && (
          <div className="fixed inset-0 z-[10001] flex flex-col bg-black">
            ...
            <div className="p-4 bg-gray-900">
              <button type="button" onClick={closeIos} className="w-full py-3 rounded-xl bg-white text-gray-900 font-semibold text-sm">Close</button>
            </div>
          </div>
        )}

        <div className="text-center mb-8">
          <h1 className="text-xl sm:text-2xl font-bold text-primary-navy tracking-tight">Download Your Inter Results Poster</h1>
              ...
              <div ref={previewBoxRef} className="relative w-full flex justify-center" style={{ maxWidth: PREVIEW_MAX_PX }}>
                <div className="rounded-lg overflow-hidden shadow-md border border-slate-200/80" style={{ width: scaledW, height: scaledH }}>
                  <div style={{ width: POSTER_WIDTH, height: POSTER_HEIGHT, transform: `scale(${previewScale})`, transformOrigin: 'top left' }}>
                    <InterResultsPosterPreview ref={posterRef} fullName={displayName} mobileNumber={mobile10 || undefined} />
                  </div>
                </div>
              </div>
