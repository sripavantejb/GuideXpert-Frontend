...
function drawJeePosterToCanvas(img, fullName, mobileNumber, scale = 2) {
  ...
  const sx = scale;
  const nl = JEE_POSTER_EXPORT_LAYOUT.name;
  const pl = JEE_POSTER_EXPORT_LAYOUT.phone;
...
export default function JeePosterPage() {
  ...
  const handlePng = async () => {
    ...
    setGenerating(true);
    try {
      const img = await loadJeePosterImage();
      const c = drawJeePosterToCanvas(img, displayName, mobile10, 2);
      const url = c.toDataURL('image/png');
      if (isIOSDevice()) {
        setIosResult({ url, type: 'image' });
      } else {
        const fn = `GuideXpert-JEE-Poster-${safeFilename(displayName)}-${Date.now()}.png`;
  ...
  const handlePdf = async () => {
    ...
    try {
      ...
      if (isIOSDevice()) {
        ...
        const fn = `GuideXpert-JEE-Poster-${safeFilename(displayName)}-${Date.now()}.pdf`;
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 sm:py-10">
        ...
        <div className="text-center mb-8">
          <h1 className="text-xl sm:text-2xl font-bold text-primary-navy tracking-tight">Download Your JEE Poster</h1>
              ...
              <div ref={previewBoxRef} className="relative w-full flex justify-center" style={{ maxWidth: PREVIEW_MAX_PX }}>
                <div className="rounded-lg overflow-hidden shadow-md border border-slate-200/80" style={{ width: scaledW, height: scaledH }}>
                  <div style={{ width: POSTER_WIDTH, height: POSTER_HEIGHT, transform: `scale(${previewScale})`, transformOrigin: 'top left' }}>
                    <JeePosterPreview ref={posterRef} fullName={displayName} mobileNumber={mobile10 || undefined} />
                  </div>
                </div>
              </div>
